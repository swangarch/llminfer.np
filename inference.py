...
def main():
    config = parse_json("model/config.json")
    tokens = Tokenizer.from_file("model/tokenizer.json")
    weights = load_file("model/model.safetensors")
    
    chat = """The following is a conversation between a User and a helpful Assistant.
...
Assistant:"""
    inference(chat, weights, config, tokens)
