...
class GPT2(BaseModel):
    ...
    def pred_next_token(self, ids: list, prefill: bool = False, kv_cache_enabled: bool = False,
        ...
        if not kv_cache_enabled or prefill:
            x = self.weights["wte.weight"][ids] + self.weights["wpe.weight"][np.arange(len(ids))]
        else:
        ...
        for i in range(self.config["n_layer"]):
            x1 = layer_norm(x, self.weights[f"h.{i}.ln_1.weight"], self.weights[f"h.{i}.ln_1.bias"], self.config["layer_norm_epsilon"])
            if not kv_cache_enabled or prefill:
                QKV = x1 @ self.weights[f"h.{i}.attn.c_attn.weight"] + self.weights[f"h.{i}.attn.c_attn.bias"]
                Q, K, V = np.split(QKV, 3, axis=-1)
            else:
                Wq, Wk, Wv = np.split(self.weights[f"h.{i}.attn.c_attn.weight"], 3, axis=-1)
                Bq, Bk, Bv = np.split(self.weights[f"h.{i}.attn.c_attn.bias"], 3, axis=-1)
                Q = x1 @ Wq + Bq
                K = np.concatenate([self.kv_cache[i]["K"], (x1[-1, :] @ Wk + Bk).reshape(1, -1)], axis=0)
                V = np.concatenate([self.kv_cache[i]["V"], (x1[-1, :] @ Wv + Bv).reshape(1, -1)], axis=0)

            if prefill:
                self.kv_cache.append({"K": K, "V": V})
            else:   
                self.kv_cache[i] = {"K": K, "V": V}
            Q_heads = np.split(Q, self.config["n_head"], axis=-1)
            K_heads = np.split(K, self.config["n_head"], axis=-1)
            V_heads = np.split(V, self.config["n_head"], axis=-1)
            ...
            x2 = layer_norm(x, self.weights[f"h.{i}.ln_2.weight"], self.weights[f"h.{i}.ln_2.bias"], self.config["layer_norm_epsilon"])
        ...
        x = layer_norm(x, self.weights[f"ln_f.weight"], self.weights[f"ln_f.bias"], self.config["layer_norm_epsilon"])
