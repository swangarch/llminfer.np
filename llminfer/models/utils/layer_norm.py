...
def layer_norm(x: np.array, weight: np.array, bias: np.array, eps: float = 1e-5, np=np) -> np.array:
...
def rms_norm(x: np.array, weight: np.array, eps: float = 1e-5, np=np) -> np.array:
